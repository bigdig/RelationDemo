...
+ (void)getUserInfoWithCompleteBlock:(void (^)(BOOL, BCWXUserInfo *))complete
{
    ...
    [[BCWXSocialHandler sharedInstance].networkManager GET:@"userinfo" parameters:@{@"access_token": [BCWXSocialHandler sharedInstance].accessToken, @"openid": [BCWXSocialHandler sharedInstance].openId} progress:^(NSProgress * _Nonnull downloadProgress) {
        ;
...
+ (void)refreshAccessToken:(NSString *)refreshToken
{
    ...
    [[BCWXSocialHandler sharedInstance].networkManager GET:@"oauth2/refresh_token" parameters:@{@"appid": [BCWXSocialHandler sharedInstance].appId, @"refresh_token": refreshToken, @"grant_type": @"refresh_token"} progress:^(NSProgress * _Nonnull downloadProgress) {
        ;
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (responseObject)
        {
