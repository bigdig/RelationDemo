...
- (AFHTTPSessionManager *)networkManager
{
    if (! _networkManager)
    {
        _networkManager = [[AFHTTPSessionManager alloc] initWithBaseURL:[NSURL URLWithString:@"https://api.weixin.qq.com/sns"]];
...
- (void)onResp:(BaseResp *)resp
{
    if ([resp isKindOfClass:[SendAuthResp class]])
    {
        ...
        if (aResp.errCode == WXSuccess)
        {
            ...
            [self.networkManager GET:@"oauth2/access_token" parameters:@{@"appid": self.appId, @"secret": self.appSecret, @"code": code, @"grant_type": @"authorization_code"} progress:^(NSProgress * _Nonnull downloadProgress) {
                ;
